...
const paginateArray = (dataEntries, paginateSettings) => {
  ...
  const numberOfPages =
    dataEntries.length % entriesOnPage === 0
      ? Math.floor(dataEntries.length / entriesOnPage)
      : Math.floor(dataEntries.length / entriesOnPage) + 1;

  const isInteger =
    Number.isInteger(actualPageIdx) && Number.isInteger(entriesOnPage);
  if (!isInteger)
    throw new Error('actualPageIndex and entriesOnPage have to be a integer');

  const isPositiveNumbers = actualPageIdx >= 0 && entriesOnPage >= 0;
  if (!isPositiveNumbers)
    throw new Error(
      'actualPageIndex and entriesOnPage cannot be a negative number'
    );

  const isCorrectActualPageIndex = actualPageIdx < numberOfPages;
  if (!isCorrectActualPageIndex)
