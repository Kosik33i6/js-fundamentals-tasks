const validators = {
  forString: {
    isCorrectString: (string) => {
      const isArgumentStringType =
        typeof string === 'string' && string.length >= 2;
  ...
  forArray: {
    ...
    isListOfStrings: (array) => {
      ...
      const isArrayElementsHaveCorrectType = array.every(
        (element) => typeof element === 'string'
      );
      if (!isArrayElementsHaveCorrectType)
        throw new Error('Array elements have to be a string');
    },
    ...
    isListOfProduct: (array, classObject) => {
      ...
      if (!isArrayElementsHaveCorrectType)
        throw new Error('Array elements have to be a instance of class');
    },
  },
  ...
  forNumber: {
    ...
    isCorrectIndex: (index, array) => {
      ...
      if (!isIndexInteger)
        throw new Error('Argument index have to be a integer');

      const IsIndexHaveCorrectlyValue = index >= 0 && index < array.length;
      if (!IsIndexHaveCorrectlyValue)
        throw new Error('Argument index is too low or too high');
    },
  },
  ...
  forClassInstance: {
    isInstanceOfClass: (classInstance, classObject) => {
      ...
      if (!isInstanceOfClass)
        throw new Error('Argument have to ba a instance of class');
    },
  },
};
